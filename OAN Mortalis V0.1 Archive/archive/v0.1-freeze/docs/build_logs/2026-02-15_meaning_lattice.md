...
*   **`DialecticTraceEvent`**: A generic event for meaning interactions (`DiatecticEventType`).
...
```bash
# Propose spans
Oan.Cli meaning propose --session cli-session --text "Manage the project."

# Update a span
Oan.Cli meaning update --session cli-session --span <ID> --status Confirmed

# Set FrameLock
Oan.Cli meaning framelock --session cli-session --goal "Project Management" --mode Plan

# Check Risk
Oan.Cli meaning risk --session cli-session
```
